...
labels <- list(age_cent = "Age, years",
...
var_label(df_scaled) <- labels
var_labels(df_scaled, labels_vec)
...
model_plot <- plot_models(list(GEE = m5, "robust LM" = m2_robust),
           ...
           legend.title = "Models", show.legend = FALSE,
           show.values = TRUE, axis.labels = labels_vec,
           spacing = 0.6, value.size = 1.8, line.size = 0.7,
           dot.size = 2, axis.lim = c(-0.8, 1.5)) +
  theme(panel.grid = element_blank(), text = element_text(vjust = 0.2))
model_plot

fpath <- here("figures", "methylation-models-plot.jpg")
ggsave(fpath, model_plot, dpi = 600, width = 7)
```
